...
This skill is installed inside Cal's CreativeStudio project. The default workflow ("paste this prompt into the Higgsfield website") is **overridden**. Instead:

**Claude runs the Higgsfield CLI directly, then auto-logs the generation to Cal's Google Sheet tracker.** Cal never opens the website. Cal never pastes a prompt. Cal never logs manually.
...
|---|---|
| Banana Pro (Mode 1A) | `nano_banana_2` |
| Soul Cinema Step 1B.1 (outfit on neutral model) | `soul_cinematic` |
| Soul Cinema Step 1B.2 (composite onto character) | `soul_cinematic` |
| 6-panel character sheet (Mode 2) | `nano_banana_2` |
| Scene plate (Mode 3) | `nano_banana_2` (use `gpt_image_2` only if the scene has readable text on signs/labels/packaging) |
| GPT-2 face detail (Mode 4) | `gpt_image_2` |
...
1. The auto-logger prints `Logged: [client] model — filename` and the output URL.
2. Open the output: `start "" "<output_file_path>"` so Cal sees it.
3. If Cal rates it or wants notes added, pass them as the 4th and 5th args to `log-from-json.js` on the next gen, or use `tracker/mark-approved.js` to update an existing row.

The skill's prompt-writing grammar (Modes 1–4, pre-prompt confirmations, photoreal stack, etc.) is unchanged — only the **delivery mechanism** changes from "paste into web UI" to "Claude runs CLI with auto-log."
...
The flow is always: **confirm client → confirm character → confirm what's about to be prompted → deliver the prompt in a fenced code block → run the CLI command → log automatically → open the output file**.

**CreativeStudio behavior (overrides default):** Claude does not say "paste this into Higgsfield." Claude runs the CLI directly using the canonical execution pattern from the CreativeStudio CLI Integration section above. The skill's job ends when the Sheet row is logged and the output file is opened on Cal's screen.

If reference images are needed, Cal passes them via `--start-image` or `--reference-image` flags on the CLI (depending on the model), not by attaching in a web UI.

If the user requests multiple shots in one ask, deliver each prompt in its own code block, sequentially numbered or labeled — run the pre-prompt confirmation once before delivering the batch, then execute the CLI commands sequentially, logging each one.
