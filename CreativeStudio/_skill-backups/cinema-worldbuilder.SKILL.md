...
## CREATIVESTUDIO CLI INTEGRATION (READ FIRST — OVERRIDES DEFAULT DELIVERY)

This skill is installed inside Cal's CreativeStudio project. The default workflow ("paste this prompt into Seedance via the Higgsfield website") is **overridden**. Instead:

**Claude runs the Higgsfield CLI directly with `--model seedance_2_0`, then auto-logs the generation to Cal's Google Sheet tracker.** Cal never opens the website. Cal never pastes a prompt. Cal never logs manually.

**At the start of any session that uses this skill, ask once:** *"Which client is this for?"* — the answer goes into every CLI command so the Sheet row lands under the right client. If the client folder doesn't exist under `clients/`, surface that before generating.

**Hard rules (from CreativeStudio CLAUDE.md — never violate):**
- Never use the Higgsfield MCP. CLI only.
- Always pass `--json` flag for clean output parsing.
- Always pipe to `node tracker/log-from-json.js` — never skip the auto-log.
- After every generation, open the result file with `start "" "<path>"` so Cal can see it immediately.
- **For video tests, always default to `seedance_2_0 --resolution 480p --aspect_ratio 1:1`** — cheapest option until Cal approves a direction. Only upscale to 1080p once a direction is locked.
- Seedance 2.0 minimum duration is 4 seconds. `--duration 3` errors out.
...
cd "C:/Users/Aesth/Desktop/Zentara/Projects/CreativeStudio" && higgsfield generate create \
  ...
  --aspect_ratio 1:1 \
  --wait \
  --json | node tracker/log-from-json.js "<short summary, ~10 words>" "seedance_2_0" "<client_name>"
```

**Resolution / aspect overrides:** only when Cal has approved the look and asks for the final-quality render. Default test pass is always 480p / 1:1 / shortest viable runtime.

**For multi-shot sequences:** Seedance generates one continuous clip per CLI call. If the Dynamic Description contains hard cuts, that's the model's job to handle internally — still one CLI call, one logged row. For sequences that require separate generations stitched in post (different reference images per shot, mode-stacking), run one CLI call per shot, log each, then stitch with FFmpeg per CreativeStudio CLAUDE.md.

**After the command completes:**
1. The auto-logger prints `Logged: [client] seedance_2_0 — filename` and the output URL.
2. Open the output: `start "" "<output_file_path>"` so Cal sees it.
3. If audio needs stripping: `ffmpeg -i input.mp4 -an output.mp4`.

The skill's prompt-writing grammar (5 cinema modes, pre-prompt confirmation, diegetic audio rule, etc.) is unchanged — only the **delivery mechanism** changes from "paste into web UI" to "Claude runs CLI with auto-log."
...
The flow is always: **confirm client → session-opener character gate (once) → pre-prompt confirmation → deliver title line + code block → run the CLI command → log automatically → open the output file**.
...
Deliver the prompt inside a single fenced code block — no preamble inside the code block, no postamble unless the user explicitly asks for breakdown or rationale.

**CreativeStudio behavior (overrides default):** Claude does not say "paste this into Seedance." Claude runs the Higgsfield CLI directly with `--model seedance_2_0` using the canonical execution pattern from the CreativeStudio CLI Integration section above, then logs the gen and opens the output file on Cal's screen.

If reference images are needed (locked character, start-frame anchor), Cal passes them via `--start-image` on the CLI, not by attaching in a web UI.

If multiple shots are requested, deliver each prompt in its own code block, sequentially numbered or labeled by shot. Run the pre-prompt confirmation once before the batch, then execute the CLI commands sequentially, logging each one. Stitch with FFmpeg per CreativeStudio CLAUDE.md when finishing.
