...
- **Carousel format is ALWAYS 1:1 (square). HARD RULE.** Every carousel slide, every client, every time — generate at 1:1 aspect ratio, never 4:5 or any other ratio. (Locked by Cal, June 3 2026.)
- **Always write carousels BACKWARDS. HARD RULE.** Lock the FINAL slide first (the payoff and CTA — the one thing the viewer should remember and do), then write the hook slide to set that payoff up, then fill the middle slides to carry them from hook to payoff. Every slide must earn the next swipe and point toward that ending. No wandering, no dead slides. (Locked by Cal, June 3 2026.)
...
- **NEVER have a skirt, dress, or any garment pulled, lifted, or bent up. HARD SAFETY RULE.** Especially on kids. Never write a prompt that asks to "reveal," "show," or "lift" what is underneath a garment (built-in shorts, lining, etc.) — the model will make the character pull the garment up, which is inappropriate and unnatural. Underneath features (built-in shorts, etc.) are shown ONLY in a still product photo or named in the voiceover, NEVER revealed in motion. Keep hands away from the garment at all times. Garment movement comes only from natural body motion like walking.
- **NEVER let the video model burn in on-screen text, captions, or brand words. HARD RULE.** We always use our own end card for branding. Putting brand words (like the brand name) in the prompt makes Seedance render them as ugly on-screen text. Always add "no text, no captions, no words or letters anywhere on screen" to every video prompt. Keep the brand name OUT of any line the model might render — put it on the end card only, never in the prompt's spoken/visual text.
- **NEVER a close-up or isolated shot of a kid's rear, backside, or the back of a skirt/pants. HARD SAFETY RULE.** No zooming, no low angles, no camera lingering on the lower back or seat. The back of a garment is shown ONLY in a still product photo, or as a wide, full-body, eye-level walk-away where the whole child and the environment are in frame and the action is natural (walking to class). Distance and full body = fine. Close and isolated = never. Be more creative than filming a kid's behind.
- **NEVER make a kid in a skirt or dress crouch, squat, kneel, sit on the ground, or bend low. HARD SAFETY RULE.** (Confirmed on the UNIK GU07 playground gen, June 2 2026 — a crouch beat rode the skirt up and put the camera low on the lower body. Inappropriate, and exactly what the rules above forbid.) Crouching/squatting makes the skirt ride up AND pulls the camera down toward the lower body — two violations at once. Keep kids in skirts STANDING and WALKING only. The garment's detail (pleats, swing, back panel) shows through walking motion, never through crouching. If a beat seems to need a low pose, rewrite it as a standing or walking beat instead.
...
9. **Never let a smile hold. HARD RULE.** A frozen grin held the whole clip is the #1 AI tell. Smiling is fine, but it must come and go like a real person — write it as "a smile flickers up then fades," "a real smile breaks for a second then softens," never a fixed grin at the lens. Always pair with eyes that wander or drift off, never locked on the camera. Applies to every character, every client, every clip.
   - **PROVEN FIX (Unik GU07, Jun 2026 — researched + confirmed on camera):** The held smile comes FROM THE REFERENCE PHOTOS. Seedance copies the expression straight off the reference image and treats it as part of the character's identity — text instructions like "do not smile" CANNOT override a smiling reference. Two reliable fixes: (a) **Give the characters a real task to concentrate on** — a jammed locker they have to work open, drawing, reading a schedule, looking for a room number. The moment their attention is on a task instead of the camera, the posing stops. Task-focus beat every "do not smile" wording we tried. (b) **The complete fix is neutral-face reference photos** — regenerate the character's reference with a calm, mouth-relaxed expression, then text can finally steer the face. Use (a) always; reach for (b) when you need their faces toward camera and still no grin.
10. **Characters must ALWAYS be in motion. Never have them stand still and pose. HARD RULE.** The instant you ask a character to "stand," "hold," or face the camera so a product is visible, they freeze into a dead catalog pose and grin at the lens. To show a product, show it THROUGH motion — the garment swinging as they walk, shifting as they turn to a friend — never by parking them still for the camera. Open the clip already mid-action ("the take opens already moving with..."), never on a static establishing pose. If a product detail needs a still, clean, held shot, that is a separate PRODUCT-ONLY clip with no person in it — never achieved by making a person pose. Product detail = product photos or product-only clips. People = always moving, always real.
